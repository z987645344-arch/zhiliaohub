...
知了hub 是用于整理和展示个人作品、学习心得、反馈交流与持续探索的多页面网站，公开地址为 [https://zhiliaohub.com](https://zhiliaohub.com)。当前版本以仓库的 Git 标签为准（`git tag --sort=-v:refname | head -1`）；仓库已完成面向唯一前置gateway的新部署配置，生产实际运行版本不在本文档中断言。
...
- 作品分组由数据库驱动，默认不预置分组、由管理员自行创建；支持按实际分组生成分类二级页、媒体展示、逐条追加/删除的更新记录时间线和按作品开关控制的公开下载。
...
- 配套 Android App 位于独立仓库 `zhiliaohub_app`，版本以该仓库 Git 标签现查为准。

## 仓库结构

- 仓库根目录只保留手写的 `index.html`；`css/`、`js/`、`assets/` 是仓库资产。其余前台页面（`works.html`、`notes.html`、`feedback.html`、`tools.html` 及全部详情页、分类页）均由发布生成，不在仓库里。
