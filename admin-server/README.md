...
4. 若要显示知天备份状态，在两端配置相同的 `ZHITIAN_OPS_TOKEN`；未配置或知天不可达时，只降级知天状态卡片，不阻断本后台。
5. 在本目录运行 `npm install`，再运行 `npm start`。
6. 浏览器访问 `http://127.0.0.1:3001/admin/login`。
...
- 生成根目录 `works.html`、按数据库中实际分组生成的 `works-category-<slug>.html` 分类页、`tools.html`、`notes.html`、`feedback.html` 及全部 `works-<slug>.html`、`notes-<slug>.html` 详情页；0 个分组时不生成分类页。
...
- 全量发布只清理带生成标记且已不在数据库中的详情页、分类页和受控作品媒体副本，不修改手写的 `index.html`、`css/` 或 `js/`；`tools.html` 自 v3.2 起由发布无条件生成并带生成标记，0 条勾选作品时仍生成空状态页，`feedback.html` 同样属于生成目标。
...
“编辑作品重构”阶段一已为 `works` 增加封面、下载、体验链接、主媒体、辅图和版本日志共8个可空字段；当时曾把现有作品分类迁为“程序/影视/生活”三个固定值。2026-09-11 起，分组改为数据库驱动且默认不预置，由管理员自行创建名称、分类页文案、排序、可见性及 URL 标识；slug 由管理员手填并严格使用与发布校验一致的 `^[a-z0-9]+(?:-[a-z0-9]+)*$` 规则。⚠️ 删除分组会级联删除其下全部作品，包括 Markdown 正文与已发布页面；这是用户于2026-09-11明确选择的行为，理由是当时作品价值低。阶段二已把作品字段接入独立表单：封面可拖动选区内部调整位置、拖动四角按16:9缩放，主媒体、多个辅媒体和ZIP可通过现有受保护上传接口写入；`detail_body` 是可空的 Markdown“详情”正文，发布时安全渲染为媒体下方的作品说明书。**2026-09-14 起「更新记录」改为独立表 `work_updates`**（`work_id` 外键 `ON DELETE CASCADE`、`recorded_at`、`body`、`created_at`）：后台在已保存的作品上逐条添加（记录时间可回填过去）与删除（二次确认），不提供编辑；前台按 `recorded_at` 倒序、同时间按 `id` 倒序渲染时间线；增删后立即全量发布。`works.version_log` 列**保留但冻结**——迁移 `works-update-timeline-v1` 把每个非空 `version_log` 原样转成一条 `recorded_at = updated_at` 的记录，此后不再读写该列。作品 Markdown 文件降为**派生快照**：仅在时间线增删时由 `work_updates` 重建（无条目时写"暂无更新记录"），任何渲染不再读它，留着只为让 `content/works/` 备份里仍有可读全文。后台CSP只为该原生表单脚本开放同源脚本，并只为本地图片裁剪预览额外开放 `blob:` 图片源，不允许内联或第三方脚本。
...
作品表单采用“先上传、后保存”，所以直接关闭页面、替换媒体或删除作品，都可能让原始文件留在 `uploads/` 而不再被记录引用。使用以下命令盘点：

```bash
# 默认只报告，不删除
npm run orphan-uploads

# 明确确认清理盘点出的旧孤儿
npm run orphan-uploads -- --delete
```

盘点不硬编码作品字段：它会检查SQLite全部业务表的所有文本/二进制值，并扫描 `content/` 与 `lab-storage/` 中的文件内容，因此作品封面、主媒体、辅图、下载ZIP、版本日志、日记Markdown、小作坊内容及未来新增字段中的引用都会被保守识别。文件名只要在任一来源出现，就不会被当成孤儿。

默认且最低保护窗口为24小时（`ORPHAN_UPLOAD_MIN_AGE_MS=86400000`）；刚上传但尚未保存的文件不会进入孤儿清单，低于24小时的配置会被拒绝。命令行默认只盘点，只有显式 `--delete` 才会删文件；另外，每日调度备份成功后会自动运行一次同样的受保护清理，备份失败当天不清理。每次删除前都会创建独立 `orphan-rollback-` 回退归档，并强制把候选ZIP实体纳入该归档；该谱系只保留最近3份，不参与手工、调度或恢复前快照轮转，也不会被调度器当成“当天已有备份”。结果持久化在 `admin.sqlite3` 的 `orphan_cleanup_state`，管理仪表盘显示最近时间、删除数量与回收体积。备份后还会重新盘点并核对大小、修改时间与SHA-256，期间发生变化或重新被引用的文件会保留。
...
后台可上传可选的16:9封面、复制访问链接、下载由现有 `lab-storage/<slug>/` 实时流式生成的ZIP、切换是否显示在 `works.html` 底部，以及删除项目。下载不落临时包、不把整包读入内存，文件名使用数据库中的原始上传名；解压产物已经进入备份并可直接恢复，因此无需额外保留源ZIP。可见项目按作品 `portfolio-card` 样式展示，无封面时使用几何占位图，多于一项时使用可拖动横向轨道。显示状态变化与删除会触发全量发布；没有可见项目时不生成空的小作坊区块。真实项目目录由 `.gitignore` 排除，只保留 `lab-storage/.gitkeep`。
...
⚠️ `LAB_BASE_URL` 在生产环境必须改为实际公开地址。若 `NODE_ENV=production` 时仍含 `localhost`，服务会打印醒目警告但不会拒绝启动。
...
- 单个调度周期首次失败后会在当天固定重试两次（5分钟、30分钟退避）；三次尝试均失败，或下一次等待会跨过下一个UTC+8调度边界时，才排到下一周期。成功后“连续失败”计数立即归零；进程重启时仍会先检查并补做当日缺失备份。
- 管理仪表盘并列显示知了hub与知天的调度备份状态；服务端统一返回 `{ zhiliaohub, zhitian }` 聚合结果，供管理页面与配套App复用。知了hub状态按调度器同一套UTC+8活动边界判断为 `ok / stale / disabled / unknown`，边界后的固定宽限期为代码常量7200秒；它用于吸收容器没有运行的窗口，不是备份耗时阈值，因此不提供可漂移的配置旋钮。精确时间保留在已认证API中供配套App复用，但不在管理仪表盘渲染；公开 `/health` 不包含任何备份状态。
- 知天状态由知了hub单向拉取 `http://zhitian-api:8000/ops/backup-status`，使用 `X-Ops-Token` 认证；连接上界2秒、请求总上界3秒。连接失败、超时或5xx只把知天卡片标为“不可达”；知天返回200但状态为 `unknown` 时显示“未知”，不会误写成“已超期”，知了hub自身卡片与管理页面继续工作。
- 备份失败时会在服务日志里打印以 `[backup]` 开头的错误，并明确写出“现在没有产生新的备份”。**如果你在日志里看到这类信息，说明备份已经停摆了，需要处理。**

⚠️ **本系统没有、将来也不会有“备份失败时主动通知”这件事。**备份失败后，界面状态会因当前活动窗口没有成功归档、且固定2小时宽限期耗尽而显示“已超期”，但**直到有人主动打开已认证的管理后台查看，才会知道**。状态卡片是显示，不是告警；有显示不等于有人会被叫醒。这是2026-09-04用户明确接受的已知缺口，不是待办。
...
- 按2026-09-04用户决定，不提供备份成功/失败的主动告警；管理后台状态卡片仅供人工主动查看，这一已知缺口不列为待办。
...
- 作品与日记的新增、编辑、删除，以及 SQLite、Markdown与生成静态页面的一致性和旧详情页清理；作品只能选择数据库中已有分组，分组 slug 严格匹配 `^[a-z0-9]+(?:-[a-z0-9]+)*$`，详情页简介必填。分组删除测试锁定实际作品数确认、逐条清理 Markdown、删除数据库记录并由全量发布清理分类页和详情页；该级联行为是用户于2026-09-11基于当时作品价值低作出的明确选择。
- 旧 works 表补齐8个可空字段；数据驱动分组迁移保留既有中文名或旧 slug 形式的作品归属、为未知旧值建立兼容分组，并以迁移标记保证重复启动幂等。
...
- 定时备份失败时记录明确日志并说明“现在没有产生新的备份”；同一周期按5分钟、30分钟固定退避重试，成功后失败计数清零，预算耗尽才排到下一周期，且启动补做行为保持不变。
...
当前自动化测试不覆盖外部验证器设备的时钟差异、浏览器视觉与原生裁剪拖拽的真实交互、真实小作坊子域名Cookie隔离、生产HTTPS/反向代理回归、异地备份或多进程/多实例并发。主站、登录页和健康接口已在生产域名实际可达，但这不等于上述边界都有自动化覆盖。GitHub Actions 自 `v2.8` 起会安装后台依赖并执行完整 `npm test`；容器、Nginx 与真实浏览器仍不在 CI 覆盖范围内。
...
- 已有本地手动备份/恢复、校验和、可选加密、保留策略、恢复前自动快照、进程内定时备份、当日自动重试及已认证状态展示；仍没有**真实**异地副本或服务器级灾难恢复演练。按用户明确决定，主动备份告警不会实现，不能把状态展示误认为告警。
...
- `lab-storage/` 已随常规备份和恢复前快照归档并可直接还原；服务器级恢复演练已于 2026-09-04 在生产完成两次真实往返，但仍缺少真正异地副本与容量告警。
