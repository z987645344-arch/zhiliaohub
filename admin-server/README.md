...
- 管理仪表盘并列显示知了hub与知天的调度备份状态；服务端统一返回 `{ zhiliaohub, zhitian }` 聚合结果，供管理页面与配套App复用。知了hub状态按调度器同一套UTC+8活动边界判断为 `ok / stale / disabled / unknown`，边界后的固定宽限期为代码常量7200秒；它用于吸收容器没有运行的窗口，不是备份耗时阈值，因此不提供可漂移的配置旋钮。精确时间只作为已认证界面的次要信息，公开 `/health` 不包含任何备份状态。
