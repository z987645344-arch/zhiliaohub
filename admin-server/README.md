...
文件名有三种互斥前缀；三个池分别轮转，任何一个池达到保留上限都不会删除另外两个池的归档：

- `backup-<时间戳>.tar.gz` —— 手动运行 `npm run backup` 产生的归档。
- `scheduled-backup-<时间戳>.tar.gz` —— 后台调度器产生的每日归档。它与手工池共用 `BACKUP_RETENTION_COUNT`，但各自独立保留最近 N 份。
...
这些快照有独立的保留数量（`PRE_RESTORE_RETENTION_COUNT`，默认保留 3 份）。手工池与调度池各自按 `BACKUP_RETENTION_COUNT` 保留，恢复前快照按自己的数量保留；三个池互不挤占。

### 自动备份

后台服务运行时，会在自己进程内定时做备份，不依赖操作系统的计划任务（这样在容器里跑也是同样的行为）。

- 默认在每日 **UTC+8 00:00** 触发，写到 `backups/`，使用独立的 `scheduled-backup-` 前缀。归档内容与手工备份相同，但保留与去重判据互不干扰。UTC+8是代码中的固定偏移，不读取宿主机或容器的 `TZ`，因此UTC容器不会误在北京时间08:00触发。
- **没有调度归档时立即保护**：服务启动时如果没有任何 `scheduled-backup-` 归档，会立刻创建一份，不等待下一个零点；已有手工归档或恢复前快照也不会压掉这次保护。
- **同一调度日重启不重复**：服务只以“最近一个UTC+8目标时刻之后是否已有 `scheduled-backup-` 归档”为判据，反复重启只读取目录，不创建额外状态文件。
- **手工备份不再满足调度边界**：即使当天刚运行过 `npm run backup`，调度器仍会按计划创建自己的归档。这是刻意取消的旧行为；一份冗余归档的代价低于因现场手工验证而让当天没有调度恢复点。
...
- 手工、调度与恢复前三种前缀严格互斥；手工池和调度池各自连续生成 N+1 份时，只轮转自身且不会减少另一池，恢复前池同样按独立数量轮转。
- 恢复前自动创建快照；误选旧归档后可用该快照退回，作品标题/摘要、Markdown正文与上传文件内容逐项核对一致。
- 恢复前快照创建失败时中止恢复，SQLite、Markdown与上传文件全部保持原样。
- 恢复前快照使用独立保留数量，不会被手工或调度备份的“最近N份”清理，其自身超出数量时才淘汰最旧的一份。
...
- 没有调度归档时定时备份立即创建；同一UTC+8调度日模拟重启3次仍只有一份；UTC+8次日00:00（UTC前一日16:00）创建第二份，并直接断言运行时计时器瞄准该UTC时刻而非容器本地零点。当天只有手工归档或恢复前快照时仍创建调度归档，只有当天已有调度归档才跳过。
- 定时备份失败时记录明确日志并说明"现在没有产生新的备份"，且不抛出、不静默。
- 备份生成后被复制到模拟异地目录，该副本可在本地备份目录整个丢失后独立完成恢复。
- 副本同步失败时如实报告，本地归档保持完整且仍可完成真实恢复。
- 模拟异地目录中的手工、调度、恢复前三个池按各自前缀独立清理，且与本地目录保持一致；`LocalMirrorDestination` 不修改也不删除传入的本地归档。
