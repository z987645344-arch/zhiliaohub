...
- 管理仪表盘直接显示最近一次成功的**调度**备份状态；服务端统一计算“正常 / 已超期 / 从未成功过”和相对时间，`GET /api/admin/backup-status` 向已认证管理会话返回同一结果，供配套App后续复用。`BACKUP_STATUS_OVERDUE_MS` 默认 `108000000`（30小时）；精确时间只作为已认证界面的次要信息，公开 `/health` 不包含任何备份状态。
- 备份失败时会在服务日志里打印以 `[backup]` 开头的错误，并明确写出“现在没有产生新的备份”。**如果你在日志里看到这类信息，说明备份已经停摆了，需要处理。**

⚠️ **本系统没有、将来也不会有“备份失败时主动通知”这件事。**备份失败后，界面状态会因最近成功时间不再前进而最终显示“已超期”，但**直到有人主动打开已认证的管理后台查看，才会知道**。状态卡片是显示，不是告警；有显示不等于有人会被叫醒。这是2026-09-04用户明确接受的已知缺口，不是待办。
