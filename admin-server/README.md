...
默认且最低保护窗口为24小时（`ORPHAN_UPLOAD_MIN_AGE_MS=86400000`）；刚上传但尚未保存的文件不会进入孤儿清单，低于24小时的配置会被拒绝。清理从不由后台或发布流程自动触发，只有显式 `--delete` 才会删文件；删除前先用现有手工备份机制创建回退归档，并强制把候选ZIP实体纳入该归档，即使常规备份设置了 `BACKUP_EXCLUDE_ZIP=true` 也不会留下无法回退的删除。备份后还会重新盘点并核对大小、修改时间与SHA-256，期间发生变化或重新被引用的文件会保留。
