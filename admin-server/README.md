...
最后重新启动后台（`npm start`，或 `docker start <容器名>`）。做完了。
...
| 上传文件 | 你传过的图片、视频、ZIP 等，存在 `uploads/` 目录 |
| 归档 / 备份文件 | 一个 `.tar.gz` 压缩包，把上面三样打包在一起。这就是"一份备份" |
| 恢复 | 用某个归档里的内容，**覆盖**掉现在的数据库、正文和上传文件 |
| 恢复前快照 | 恢复动作开始前，系统自动把"当前"数据也打包成一份归档，作为后悔药 |
...
- 一份备份包含：SQLite 一致性快照、`content/works/`、`content/notes/`、`uploads/`。归档内的 `manifest.json` 记录每个文件的路径、字节数与 SHA-256；恢复前逐项校验，归档里出现未声明的文件会直接拒绝恢复。
...
正式的仓库内部署配置为根目录 `docker-compose.yml` 与 `deploy/nginx.conf`，两层环境变量、独立公开站点目录、权限要求及实际部署顺序见 `deploy/README.md`。这些配置尚未在真实服务器启用。
...
- 仅完成本地开发与部署前准备；根目录已有正式Compose/Nginx配置，但尚未在真实服务器启用，没有真实域名、HTTPS验收或部署操作。
