...
| 上传文件 | 你传过的图片、视频、ZIP 等，存在 `uploads/` 目录；ZIP仍可正常上传和下载，但不进入备份内容 |
| 归档 / 备份文件 | 一个 `.tar.gz` 压缩包，包含数据库、正文和非ZIP上传；被排除ZIP只在 `manifest.excluded` 留存路径、大小与SHA-256 |
| 恢复 | 用某个归档里的内容，**覆盖**掉现在的数据库、正文和非ZIP上传；随后按清单从本地补回ZIP |
| 恢复前快照 | 恢复动作开始前，系统自动把“当前”数据也按同一规则打包；ZIP同样只留清单、不含内容 |

**备份不含 `.zip`，恢复后需由用户从本地补齐，清单见 `manifest.excluded`。一份“静默地少了东西”的备份比没有备份更危险。**因此创建和恢复命令都会明确打印排除数量；不要忽略“待补齐”提示，也不要把归档文件单独当成完整下载资源库。
...
- 默认每 24 小时一次，写到 `backups/`，和手动 `npm run backup` 产生的文件完全一样。
- **重启不会漏备份**：服务启动时会先看一眼最近一份备份是什么时候的，已经超过一个周期就立刻补一次。
- **频繁重启也不会重复备份**：如果最近刚备份过，启动时不会再来一次。
- 相关开关：`BACKUP_SCHEDULE_ENABLED`（默认 `true`）、`BACKUP_INTERVAL_MS`（默认 86400000，即 24 小时）。
...
- 一份备份包含：SQLite 一致性快照、`content/works/`、`content/notes/`、`uploads/` 下的非ZIP文件。格式2的 `manifest.json` 在 `files` 中记录入包文件，在 `excluded` 中记录未入包ZIP；两类都包含相对路径、字节数与SHA-256。恢复器兼容格式1旧归档；对格式2会校验排除项只能位于 `uploads/` 且扩展名为ZIP，归档里实际夹带排除项或出现其他未声明文件都会拒绝恢复。
- `BACKUP_RETENTION_COUNT` 默认 7；`BACKUP_DIR` 可指向服务器上的独立持久目录。
...
- 加密备份在原始SQLite、Markdown和非ZIP上传数据被破坏后完整恢复，错误密码拒绝解密。
- 同一上传池含ZIP和非ZIP时，ZIP内容不入包但路径、大小、SHA-256进入 `manifest.excluded`；解包确认ZIP缺失、非ZIP存在，恢复返回明确补齐提示，并验证格式2恢复器继续兼容格式1旧清单。
- 备份保留策略只留下最近N份归档。
...
- 定时备份到期触发、未到期跳过；重启后既不重复也不漏掉，长时间停机后重启会立即补一次。
...
- 上传ZIP由用户在本地另行保存，不进入常规备份或恢复前快照；每份新manifest会留存补齐所需的原路径、大小和SHA-256，恢复后必须按提示人工补齐。
