...
单个调度周期首次失败后会固定等待5分钟、30分钟再试，总计最多3次，且不会让重试跨过下一次UTC+8调度边界；这些重试参数刻意写在代码中，不新增运维旋钮。`BACKUP_STATUS_OVERDUE_MS` 是唯一的状态阈值配置，默认108000000毫秒（30小时）。状态只通过已认证管理仪表盘及 `GET /api/admin/backup-status` 暴露，公开 `/health` 不返回备份信息。

⚠️ **本系统没有、将来也不会有“备份失败时主动通知”这件事。**失败会让“最近一次成功”停止前进，超过阈值后管理界面会显眼显示“已超期”；但**直到有人主动打开管理后台查看，才会知道**。状态卡片是显示，不是告警；有显示不等于有人会被叫醒。这是2026-09-04用户明确接受的已知缺口，不是待办，部署者不得把页面上存在状态卡片写成“已经有监控告警”。
