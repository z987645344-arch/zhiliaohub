...
- 根目录 `docker-compose.yml`：构建 `admin-server`，运行官方 Nginx，并定义持久挂载、健康检查、应用网络及外部运维网络。
...
单个调度周期首次失败后会固定等待5分钟、30分钟再试，总计最多3次，且不会让重试跨过下一次UTC+8调度边界；这些重试参数刻意写在代码中，不新增运维旋钮。备份状态与调度器复用同一个活动边界，并使用代码内固定的7200秒宽限期；该宽限吸收的是容器没有运行的窗口，不是备份耗时，故不再提供 `BACKUP_STATUS_OVERDUE_MS`。状态只通过已认证管理仪表盘及 `GET /api/admin/backup-status` 暴露，公开 `/health` 不返回备份信息。

### 知天备份状态专用网络

`admin-server` 同时加入默认可出站的 `app-network` 与外部 `zhiliao-ops-bridge`，连接优先级分别固定为100与10；后者是由服务器基础设施预建的 `external: true` 内部网络，Compose不负责创建。这个优先级是安全与可用性要求：运维网络没有外网出口，若它取得默认路由，容器仍可能保持healthy，但所有出站请求会静默超时。Nginx不加入运维网络，因为它不需要访问知天，额外接入只会增加一条绕过反向代理的裸API通路。

知了hub只通过服务名向 `http://zhitian-api:8000/ops/backup-status` 发起 `GET`，请求头为 `X-Ops-Token`，令牌来自 `admin-server/.env` 的 `ZHITIAN_OPS_TOKEN`。方向固定为**知了hub拉取知天**，不向知天推送任何状态。客户端使用2秒连接上界与3秒总上界；知天不可达、超时或5xx只降级它自己的卡片，不能拖垮知了hub状态与管理页面。HTTP 401表示两端令牌不一致，404表示知天未配置该端点；HTTP 200但 `status=unknown` 表示知天服务可达、却无法判断其备份目录，危险性不同于网络不可达。

上线后先在后台容器内读取 `/proc/net/route`，确认默认路由不属于无外网出口的运维网络，再做一次真实出站请求。随后分别验证两张卡片；仅有容器healthy或Compose语法通过不能证明多网络默认路由正确。

⚠️ **本系统没有、将来也不会有“备份失败时主动通知”这件事。**失败会让当前活动窗口缺少成功归档，固定2小时宽限期耗尽后管理界面会显眼显示“已超期”；但**直到有人主动打开管理后台查看，才会知道**。状态卡片是显示，不是告警；有显示不等于有人会被叫醒。这是2026-09-04用户明确接受的已知缺口，不是待办，部署者不得把页面上存在状态卡片写成“已经有监控告警”。
...
托管 Unity WebGL 时建议设置 `LAB_MAX_UNCOMPRESSED_BYTES=314572800`（300 MB），并同步提高 gateway 与站内 Nginx 的 `client_max_body_size`；只改解压上限不能让更大的 ZIP 穿过前置请求体限制。仓库默认值仍保持 100MiB，不因 Unity 支持而自动放宽所有部署。
...
- 真正的异地对象存储备份；
- 管理后台备份状态仍依赖人工主动查看（用户明确接受的已知缺口，不是待办）；服务器级恢复演练已于 2026-09-04 在生产完成两次真实往返；
- 多后台实例或水平扩容；
- 由CI自动执行后台测试、容器和生产回归。
