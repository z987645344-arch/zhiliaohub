...
const works = [
  ['mix-video', '创作混剪视频', '影视', '把音乐、节奏与片段重新编排成一段个人表达。', '把音乐、节奏与片段重新编排成一段个人表达。当前详情页已建立，具体作品与制作记录仍在整理。', '这里将用于记录素材选择、节奏编排和版本复盘。当前文字仅说明页面结构，不代表日志已经发布。'],
  ['ai-music', 'AI音乐作品', '影视', '记录从旋律构思到生成编曲的声音实验。', '记录从旋律构思到生成编曲的声音实验。当前详情页已建立，试听内容与制作记录仍在整理。', '这里将用于记录旋律方向、生成尝试与编曲调整。当前文字仅说明页面结构，不代表日志已经发布。'],
  ['ai-video', 'AI视频作品', '影视', '用生成式影像探索短片叙事与视觉节奏。', '用生成式影像探索短片叙事与视觉节奏。当前详情页已建立，成片与生成过程仍在整理。', '这里将用于记录镜头设计、生成迭代与剪辑复盘。当前文字仅说明页面结构，不代表日志已经发布。'],
  ['3d-model', '建模作品', '影视', '收集角色、场景与物件的三维建模练习。', '收集角色、场景与物件的三维建模练习。当前详情页已建立，模型展示和制作记录仍在整理。', '这里将用于记录造型、材质和场景搭建过程。当前文字仅说明页面结构，不代表日志已经发布。'],
  ['web-design', '网页设计作品', '程序', '以信息层级和交互细节打磨网页体验。', '以信息层级和交互细节打磨网页体验。当前详情页已建立，案例画面与设计记录仍在整理。', '这里将用于记录信息架构、视觉推演与响应式取舍。当前文字仅说明页面结构，不代表日志已经发布。'],
  ['zhili', '知历', '程序', '面向个人记录与时间回望的软件作品。', '面向个人记录与时间回望的软件作品。当前只建立展示骨架，功能说明与开发记录仍在整理。', '这里将用于记录产品设想、界面演进与实现复盘。当前文字仅说明页面结构，不代表日志已经发布。'],
  ['zhiliao', '知了', '程序', '围绕个人知识与创作整理的软件作品。', '围绕个人知识与创作整理的软件作品。当前只建立展示骨架，功能说明与开发记录仍在整理。', '这里将用于记录信息整理方式、交互取舍与开发复盘。当前文字仅说明页面结构，不代表日志已经发布。'],
  ['zhitian', '知天', '程序', '企业知识库RAG+Agent系统', '企业知识库RAG+Agent系统。知天与知了hub 保持独立仓库和独立部署，本站当前只提供作品条目与详情占位。', '这里将用于整理知天作为作品的阶段记录。本仓库不包含知天业务逻辑、管理后台、API 或反向代理配置。', 'official_url_pending'],
  ...
  try {
    ...
    if (siteChanged) {
      ...
      for (const entry of currentEntries) {
        if (entry.isFile() && /^(?:(?:works|notes)(?:-[a-z0-9-]+)?|feedback)\.html$/.test(entry.name) && !siteSnapshots.has(entry.name)) {
