...
function labManagementPage({ csrfToken, projects = [], notice = '' }) {
  ...
  return layout({
    ...
    authenticated: true,
    csrfToken,
    content: `${noticeBlock(notice)}<section class="panel"><p class="admin-kicker">03 / 网页实验</p><h1>小作坊管理</h1><p>进入网页文件夹，选中全部内容压缩；不要压缩文件夹本身。</p><p><strong>Unity WebGL 导出要求：</strong>平台选择 WebGL，Compression Format 设为 Disabled，关闭 Decompression Fallback，模板使用 Default 或 Minimal。导出后进入文件夹选中全部内容压缩；首次点击画面后才有声音是浏览器规则。</p><form method="post" action="/admin/lab/upload" enctype="multipart/form-data" data-lab-upload-form data-upload-api="/api/admin/lab/upload" data-cover-upload-api="/api/admin/uploads" data-csrf-token="${escapeHtml(csrfToken)}"><input type="hidden" name="_csrf" value="${escapeHtml(csrfToken)}"><label for="labTitle">标题</label><input id="labTitle" name="title" required maxlength="120"><label for="labDescription">简介</label><textarea class="short-textarea" id="labDescription" name="description" required maxlength="1000"></textarea><label for="labCoverFile">列表封面（可选）</label><input id="labCoverFile" type="file" accept="image/jpeg,image/png,image/webp,image/gif,image/avif"><div class="cropper" data-lab-cropper hidden><canvas data-lab-cover-canvas></canvas><button type="button" data-lab-upload-crop>上传 16:9 裁剪封面</button></div><input type="hidden" name="coverImage" data-lab-cover-value><p class="upload-status" data-lab-cover-status role="status" aria-live="polite"></p><div class="upload-preview" data-lab-cover-preview></div><label for="labFile">ZIP包</label><input id="labFile" name="file" type="file" accept=".zip,application/zip,application/x-zip-compressed" required><label class="choice checkbox-choice"><input type="checkbox" name="isVisible" value="1"> 上传后展示在作品页底部</label><button type="submit" data-lab-upload-button>上传并生成链接</button><p class="upload-status" data-lab-upload-status role="status" aria-live="polite"></p></form></section>${projectsMarkup}<script src="/admin/lab.js" defer></script>`,
  });
}
