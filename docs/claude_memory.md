...
> **最后更新：2026-08-04**（v1.2 部署前准备与设备认证登录已存档）
...
| 定位 | 个人多作品展示网站 |
| 当前主线 | v1.2 已完成管理后台部署前准备与设备认证接口存档；后续安卓App在独立仓库开发，真实部署等待服务器条件就位 |
...
| 运行边界 | 前台仍可直接打开HTML且不发送数据；后台已有SQLite持久化session、设备认证、本地备份/恢复及Docker部署片段，但没有安卓App代码、真实服务器/域名/HTTPS、共享配置合并、异地备份或实际部署 |
...
| 项 | 说明 |
|------|------|
| 状态 | 🟢 v1.2 已完成本地开发验证与版本存档；App、异地备份和真实部署仍是独立后续工作 |
| 本轮完成 | 新增单设备管理、密码+TOTP授权的短期一次性配对码、P-256公钥登记、随机挑战/签名登录、重放防护、吊销与新设备自动替换；现有密码+TOTP继续作为并行备用入口 |
| 交互边界 | 现有前台的下载、登录、编辑、反馈、评论、跟评和知天正式入口仍全部未开放；后台仅在本地服务内可用，前台尚未读取后台内容 |
| 验证结果 | `npm test` 共28项（含具名子用例）全部通过；设备流程覆盖配对码权限/过期/重用、公钥存储、正确/错误签名、挑战重放/过期、吊销、重新配对与自动替换，原有密码+TOTP及部署前准备测试全部回归通过 |
...
- 配套安卓App在独立仓库 `zhiliaohub_app` 中开发，待仓库建立后按 `admin-server/README.md` 的设备接口协议进行对接；本仓库不加入App代码。
...
- 安卓App对接：`zhiliaohub_app` 仓库建立后验证Android Keystore生成P-256密钥、手动输入配对码、DER签名、Cookie持久化及服务端错误处理。
