...
> **最后更新：2026-08-07**（v1.4：作品/日记保存即静态发布与App局域网联调支持已存档）
...
| 定位 | 个人多作品展示网站 |
| 当前主线 | 原手写的8个作品、3篇日记已迁入SQLite/Markdown；后台保存即全量生成静态列表与详情页，局域网联调入口已完成可信网络真机验证 |
...
| 项 | 说明 |
|------|------|
| 状态 | 🟢 v1.4已存档：静态发布、真实11条内容迁移及App同一WiFi联调支持完成；异地备份和真实部署仍是独立后续工作 |
| 本轮完成 | 新增稳定slug、Markdown渲染模板、全量发布/旧页清理、发布状态、保存即发布和一次性迁移；新增可选局域网监听与多网卡地址探测；原8个作品与3篇日记已由后台数据管理 |
| 交互边界 | 前台仍是零运行时后端请求的静态HTML；下载、前台登录/编辑、反馈、评论、跟评和知天正式入口仍全部未开放，内容编辑只通过本地管理后台进行 |
| 验证结果 | 后台语法检查与34项自动化测试通过；真实SQLite为8/3且完整性检查正常，完成新增→编辑→删除回归；13个生成页在1280px与390px浏览器巡检中无横向溢出、无控制台警告或错误；Vivo真机在无adb reverse下完成局域网登录、断网提示与恢复验证 |
...
| `admin-server/src/services/publish-service.js` / `src/templates/` | 从SQLite与Markdown全量生成作品/日记列表和详情页；只清理带生成标记的过期详情页，并记录最近发布时间 |
