...
| 安卓App位于独立仓库 | `zhiliaohub_app` 走**独立标签线**，当前 `v0.3.1`（纯文档与 `.gitignore` 整理，功能基线仍是 `v0.3`）；已完成prod/qa并装、设备认证、会话隔离与网络容错真机验证；本仓库只保留服务端协议和兼容性说明，不包含Android代码 |
...
**服务端协议与独立App均已实现。** `zhiliaohub_app` 的**功能基线 `v0.3`** 已在真实安卓设备验证配对、挑战签名、Cookie持久化、吊销、覆盖安装、prod/qa双包隔离及只读请求网络容错（当前标签为 `v0.3.1`，那一轮是纯文档与 `.gitignore` 整理，未改代码）；本仓库不包含Gradle、Android源码或发布配置。两仓库版本各自独立，通过 `admin-server/README.md` 记录最低兼容关系；二维码扫描仍不属于当前能力。
