...
> **最后更新：2026-09-04**（**最新Git标签以 `git tag` 现查为准，本文不复述具体号**；生产运行版本只有统筹师看得到，本文不作断言；配套 App 独立标签线。本轮改写 5.5 与 9.7：CI 自 v2.8 起真跑 `npm test`；生产拓扑由 Cloudflare 代理改为单公网 IP + `zhiliao-gateway` 唯一前端，知了hub 的 Nginx 降级为只绑回环的纯 HTTP 后端）
>
> ✅ **9.7 描述的 gateway 拓扑已于 2026-09-04 完成迁移并在生产生效**（据统筹师现场验证转述）：线上运行 `zhiliaohub v3.1` + `zhiliao-gateway v0.2`，七个容器 healthy，流量全部直连新机、**Cloudflare 代理已关闭**，响应由 1.6 秒降至 0.2 秒。旧机 2026-09-13 到期，只作退路、不接流量。
> ⚠️ 本文仍不得据仓库状态推导线上状态——上述结论来自统筹师，不是本地推导。
...
6. CI 检查确定性的语法、本地引用错误与完整后台回归（`npm test`），但不把浏览器回归或后端部署伪装成自动化能力。
...
├── works.html                  # 后台生成的一级索引，按数据库中已创建的分组分区
├── works-category-<slug>.html  # 每个分组一个二级页，按数据生成；0 个分组时一个都不出
│                               # （旧的 program/film/life 三个是代码常量，2026-09-11 已改为数据驱动）
...
├── tools.html                  # 后台生成的智能工具页（列出勾选 show_on_tools 的作品）
...
├── .github/
│   └── workflows/
│       └── ci.yml              # 完整后台 npm test（v2.8 起）+ JavaScript语法 + HTML本地引用检查
├── docs/
...
| `works-{slug}.html` | 后台生成的作品详情 | Steam风格主媒体/辅图展示、标题简介、按作品配置的真实下载与体验入口、按时间倒序的更新记录时间线（表 `work_updates`）| 前台登录、下载权限控制或运行时远程内容 |
...
- 作品详情页媒体舞台（2026-09-20 重做）：缩略条**第一位是主图/主视频**，点任一缩略图切换舞台（图↔视频互切、视频切走暂停）；两侧滚动按钮未溢出置灰、到头置灰；方向键与触摸滑动；仅主媒体无辅图时不渲染缩略条。以及登录、编辑、回复和知天展示入口等通用未开放按钮的本地状态提示。
...
全部前台页面（仓库里只有手写的 `index.html`，其余均由发布生成）都不发送内容读取请求，也不依赖Cookie、localStorage或服务端会话；生成完成后即使后台停止，访客仍可阅读全部已发布内容。唯一公开写入是访客主动提交反馈留言/回复：同源POST进入SQLite pending队列，不会即时改变当前静态页。管理员审核操作只改数据库，明确执行全量发布后才用approved-only查询更新 `feedback.html`。`tools.html` 自 `v3.2` 起同样由发布生成（列出勾选 `show_on_tools` 的作品），与其他生成页一样不包含任何写入能力。
...
知了hub 的Compose/Nginx反向代理拓扑是仓库中的正式配置。**gateway 版本已于 2026-09-04 在生产生效**；此前的「Cloudflare代理 + 本项目Nginx自行终止HTTPS」那一版已随迁移退役，仅在旧机上作为退路保留至 2026-09-13。引用历史记录时须分清这两版。该事实不改变知天的独立边界：本仓库仍不代理、托管或共享知天的代码、账号和数据。两个项目的协作上下文必须分别依据对应仓库的项目记忆恢复；指挥师每项目一名、只管本项目，不能把一个项目的状态或授权带入另一个项目（权威分工见仓库外的《团队角色条例》）。
...
- `works`：标题、稳定slug、作品日期、分类（外键 `work_categories`，数据驱动）、摘要、必填详情简介（**2026-09-20 起上限 100 个码点**，保存时校验，旧超长记录仍可打开）、特殊状态、占位/排序字段、Markdown路径和时间戳；封面、下载开关/文件、体验链接、主媒体类型/路径、辅图JSON 已贯通后台独立作品表单、发布服务与前台作品模板。`version_log` 列已冻结（见 `work_updates`）。**`detail_body`（2026-09-20 起，迁移 `works-detail-body-v1`，可空）**：作品「详情」Markdown，走心得同一套渲染与转义，前台在媒体下方通栏渲染，为空整块不出。
- `work_updates`（2026-09-14 起）：作品更新记录时间线，`work_id` 外键 `ON DELETE CASCADE`、`recorded_at`（用户填、可回填）、`body`（Markdown）、`created_at`，索引 `(work_id, recorded_at DESC, id DESC)`。**是更新记录的唯一真源。** `works.version_log` 自迁移 `works-update-timeline-v1` 起冻结：保留列与历史值、不再读写。作品的 `markdown_path` 文件降为派生快照——只在时间线增删时由本表重建、无条目写"暂无更新记录"，任何渲染不读它，存在的意义只是让 `content/works/` 备份可读。日记的 Markdown 正文语义不变。
...
- `lab_projects`：唯一slug、标题、简介、原始ZIP文件名（**2026-09-20 起上传路由按 latin1→utf8 解码，迁移 `lab-filename-latin1-fix-v1` 只修满足乱码特征的历史行**）、前台显示开关、**`cover_image`（2026-09-20 起，迁移 `lab-cover-v1`，可空，走 `covers` 目录与作品同一裁剪器）**与创建/更新时间。实际解压文件保存在 `lab-storage/<slug>/`，不写入SQLite、不进入Git；**源 ZIP 解压即删、不保留**，后台「下载 ZIP」由 `GET /admin/lab/:id/download` 用 `archiver` 现场流式打包目录、不落盘（遗留 H3 据此关闭）。
- `orphan_cleanup_state`（2026-09-20 起）：上传孤儿自动清理的最近一次运行结果（时间、删除数、回收字节），单行表，供仪表盘「上次自动清理」显示。
...
跨SQLite、Markdown、上传目录和`lab-storage`不存在统一事务。SQLite在线快照本身一致，但要获得这些数据的单一恢复点，人工灾难备份应先暂停写入，最好停止服务；恢复必须在服务停止时进行。当前已实现手动备份/恢复、完整性校验、可选加密、保留策略、恢复前快照和进程内定时调度；调度器用固定UTC+8解释 `BACKUP_SCHEDULE_LOCAL_TIME`，默认每日00:00；**去重与「空目录启动即备份」的判据只认 `scheduled-backup-*`**，手工归档与恢复前快照都不再满足当天的调度边界。此前判据锚在共用前缀上，现场手工验一次归档格式就会顶掉当天的调度备份，且无任何提示。`BACKUP_MIRROR_DIR` 只会复制到同机目录，尚未实现真实异地副本。⚠️ **外部失败告警不是待实现项，而是用户 2026-09-04 明确接受的已知缺口**——改为在已认证界面显示备份状态、由人工查看代替；**有显示不等于有告警**。服务器灾难恢复演练已于 2026-09-04 在生产完成两次真实往返。

### 9.7 生产容器与反向代理

仓库提供 `admin-server/Dockerfile`、`.dockerignore`、根目录 `docker-compose.yml`、根目录 `.env.example`、`deploy/nginx.conf` 和 `admin-server/deploy/README.md`。**本节描述的 gateway 版本已于 2026-09-04 部署并在生产运行**（据统筹师现场验证：七容器 healthy、恢复往返跑通两次、真实访客 IP 贯通到后端）；上一版（本项目Nginx自行终止HTTPS、置于Cloudflare代理之后）已退役，仅在旧机保留至 2026-09-13 作退路。**「仓库已完成」与「生产已生效」仍是两件事，后者一律以统筹师口径为准，不由本文推导。** `admin-server/deploy/README.md` 记录部署拓扑、日常代码更新以及首次部署/迁移/灾难重建流程。
...
**剩余备份缺口：当前只有服务器本地归档和同机目录副本。** SQLite、Markdown、上传池与 `lab-storage` 已进入同一恢复链路；真实异地副本、加密密码托管与容量监控仍须补齐；服务器恢复演练已于 2026-09-04 完成。⚠️ **备份失败的外部告警已被明确排除**（用户 2026-09-04 决定，理由是认知负担），改为人工查看已认证界面的状态——**这是已知缺口，不是待办**。在异地副本等能力完成前，不能把本地归档视为完整灾难恢复方案。
...
| 内容全部来自数据库 | 2026-09-11 起仓库不再保留任何占位页；日记与作品有多少条、叫什么，完全取决于管理员在后台建了什么。空库时发布出的是干净空页 |
...
| 安卓App位于独立仓库 | `zhiliaohub_app` 走**独立标签线**，当前标签以该仓库 `git tag` 现查为准；**功能基线是 `v0.3`**；已完成prod/qa并装、设备认证、会话隔离与网络容错真机验证；本仓库只保留服务端协议和兼容性说明，不包含Android代码 |
...
**正式仓库配置已实现，并已于 2026-09-04 部署到生产——两者现在是同一套配置。** 根目录 `docker-compose.yml` 与 `deploy/nginx.conf` 描述后台、主站静态文件和小作坊主机拓扑，现已改为 gateway 之后的回环HTTP后端；`admin-server/deploy/README.md` 说明两层 `.env`、目录权限、日常更新及重新部署步骤。主站经 `https://zhiliaohub.com` 提供服务，2026-09-04 起由新机上的 gateway 拓扑承接。⚠️ **「仓库已完成」与「线上已生效」始终是两件事**——本次只是恰好一致，后续任何改动仍不得据仓库状态宣称已上线；生产实际运行的版本只有统筹师看得到。真实服务器路径、端口、密钥和证书位置仍只存在于服务器现场文件。知了hub 与知天继续使用独立服务、账号和数据边界。

### 11.6 自动化验证扩展

**基础检查与完整后台回归已实现，高阶检查尚未实现。** 当前CI包含 HTML 本地引用检查、JavaScript 语法检查，以及自 `v2.8` 起真实执行的完整 `npm test`（此前长期「假绿」——只做前两项，`npm test` 命中 0 次）。如果页面数量或交互复杂度继续增加，可再讨论无障碍扫描或多视口回归；性能与视觉测试不属于当前CI能力，引入前必须先评估执行环境、稳定性和维护成本。

### 11.7 备份运维演进

**本地机制和生产进程内定时触发已实现，异地容灾仍未实现。** 当前脚本已覆盖SQLite、Markdown、上传池和`lab-storage`的完整性清单、可选ZIP排除留痕、SHA-256、可选加密、恢复前快照和真实恢复验证；保留策略按手工/调度/恢复前**三条互斥谱系各自独立轮转最近N份**，任一谱系的归档都不会挤掉另一谱系；调度器随后台进程运行，按固定UTC+8时钟每日触发，不依赖容器时区。上传ZIP默认入包；如现场为节省空间启用排除，则原件由用户在本地另行保存。后续仍要接入真实异地目标、密钥托管与容量监控；定期服务器恢复演练已于 2026-09-04 首次在生产完成。⚠️ **外部告警不在后续计划内**——用户 2026-09-04 明确排除，改为人工查看已认证界面的备份状态，**有显示不等于有告警**。

### 11.8 独立安卓App对接

**服务端协议与独立App均已实现。** `zhiliaohub_app` 的**功能基线 `v0.3`** 已在真实安卓设备验证配对、挑战签名、Cookie持久化、吊销、覆盖安装、prod/qa双包隔离及只读请求网络容错（当前标签以该仓库 `git tag` 现查为准；`v0.3` 之后若只有文档与构建整理，功能基线不变，未改代码）；本仓库不包含Gradle、Android源码或发布配置。两仓库版本各自独立，通过 `admin-server/README.md` 记录最低兼容关系；二维码扫描仍不属于当前能力。
