...
> **最后更新：2026-08-13**（正式Compose/Nginx部署配置已入工作区；尚未实际部署）
...
根目录放置可直接访问的 HTML 页面；`css/`、`js/`、`assets/` 只存放跨页面共享或静态资源。根目录仍不存在前端 `src/`、`dist/` 或 `package.json`。后端的 `package.json` 和 `src/` 只存在于 `admin-server/`。仓库已提供正式Compose/Nginx配置，但它们尚未在真实服务器执行；部署时使用独立的公开站点目录，绝不能把整个Git工作区直接挂载给Nginx。
...
| 预览方式 | 前台可直接打开HTML；后台需在 `admin-server/` 配置环境变量并启动Node服务；Dockerfile与正式Compose/Nginx配置尚未实际部署 |
...
反向代理拓扑已作为正式配置进入仓库；真实域名、证书和服务器启用仍属于 Phase B 部署工作。两个项目的指挥师也属于不同协作线程，各自依赖本仓库的 `claude_memory.md` 恢复状态，不共享对话上下文。
...
### 9.7 Phase B容器与代理准备（正式配置已准备，尚未部署）

仓库现已提供 `admin-server/Dockerfile`、`.dockerignore`、根目录 `docker-compose.yml`、根目录 `.env.example`、`deploy/nginx.conf` 和 `admin-server/deploy/README.md`。它们是可继续用于服务器部署的正式配置，不再只是需要人工拼接的片段；但本轮没有连接服务器、构建镜像或启动服务，也没有真实域名、证书和上线结果。
...
5. 小作坊使用独立HTTPS主机名和只读静态目录，不反向代理到Node；真实DNS、证书范围及Cookie边界仍须部署后验证。
...
| 小作坊子域名隔离尚未实测 | localhost已验证ZIP安全、静态访问、CSP和响应不设置session Cookie；占位变量 `<LAB_SERVER_NAME>` 对应的真实DNS、HTTPS、父域Cookie边界与独立Nginx静态服务必须在部署阶段复测 |
| 生成页不可手改 | 作品/日记/反馈生成页首行带自动生成标记；应修改后台数据、审核状态或模板后重新全量发布 |
| 没有自动化构建和浏览器测试 | CI只检查JavaScript语法和HTML本地引用；视觉与交互仍需按任务进行真实浏览器回归 |
| 后台尚未部署 | 已有Dockerfile与正式Compose/Nginx配置，但没有真实域名、服务器、HTTPS、证书或这套正式拓扑的容器运行验证 |
...
**三阶段已经实现，生产部署尚未完成。** 当前完整链路为：公开同源API接收顶层留言或对approved顶层留言的回复，蜜罐和按IP限流在入口防滥用；服务端校验称呼、可选邮箱、正文长度及两层深度，所有访客提交统一写入pending。管理员在受会话和CSRF保护的论坛式后台按主题通过、拒绝/隐藏或发布固定“站长”回复；审核操作只改SQLite。全量发布时SQL从源头只选择approved公开列，生成静态 `feedback.html`，邮箱/IP、pending和rejected永不进入模板输入。
...
**正式仓库配置已准备，真实环境尚未实现。** 根目录 `docker-compose.yml` 与 `deploy/nginx.conf` 已描述后台、主站静态文件和小作坊静态主机的部署拓扑；`admin-server/deploy/README.md` 说明两层 `.env`、目录权限、初始化与验收步骤。所有真实域名、服务器路径、端口、环境变量和证书位置仍通过服务器现场文件提供，不修改仓库内配置。知了hub 与知天继续使用独立服务、账号、数据和子域名边界。
...
**本地功能与安全预检已实现，域名隔离尚未验证。** 部署时按 `admin-server/deploy/lab-subdomain.md` 为占位变量 `<LAB_SERVER_NAME>` 建立不反向代理Node的独立静态Nginx `server` 块，配置真实DNS、HTTPS证书与持久化 `lab-storage/` 绝对路径，并把 `LAB_BASE_URL` 切换为对应HTTPS子域名。上线验收必须用真实浏览器确认后台Cookie保持host-only、不会发往小作坊子域名，同时复核CSP、MIME、缓存与下载行为；未完成这些检查前不得宣称子域名隔离已经上线。
