...
> **最后更新：2026-08-11**（小作坊本地ZIP发布完成；真实子域名隔离待部署验证）
...
| 小作坊子域名隔离尚未实测 | localhost已验证ZIP安全、静态访问、CSP和响应不设置session Cookie；真实 `lab.zhiliaohub.com` 的DNS、HTTPS、父域Cookie边界与独立Nginx静态服务必须在部署阶段复测 |
...
本章记录未来决策位置与已完成的重要演进；每项明确标注“已实现”“已准备”或“尚未实现”，不能把本地功能和仓库片段误写成真实上线能力。
...
**本地功能与安全预检已实现，域名隔离尚未验证。** 部署时按 `admin-server/deploy/lab-subdomain.md` 为 `lab.zhiliaohub.com` 建立不反向代理Node的独立静态Nginx `server` 块，配置真实DNS、HTTPS证书与持久化 `lab-storage/` 绝对路径，并把 `LAB_BASE_URL` 切换为HTTPS子域名。上线验收必须用真实浏览器确认后台Cookie保持host-only、不会发往小作坊子域名，同时复核CSP、MIME、缓存与下载行为；未完成这些检查前不得宣称子域名隔离已经上线。
